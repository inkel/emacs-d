...
#+begin_src shell :tangle ~/bin/git-cleanup
  #!/usr/bin/env bash
  # set -x
...
#+begin_src emacs-lisp
  ...
  (defun inkel/jsonnet-lib ()
    (let
      ...
      (if dtroot
          (progn
            (setopt--set 'jsonnet-library-search-directories
...
** AI
Yes, it's time to accept that AI is not important. At work I've been trying [[https://www.anthropic.com/claude][Claude Code]], so let's start with it:
...
I might also try the [[https://pi.dev/][Pi coding agent]], so let's see how it integrates:
...
*** MCP
[[https://modelcontextprotocol.io/docs/getting-started/intro][Model Context Protocol]] is the new standard to communicate with AI applications.
...
#+begin_src emacs-lisp
  (use-package mcp
    ...
    ;; we could start all servers with Emacs, but we won't for now.
